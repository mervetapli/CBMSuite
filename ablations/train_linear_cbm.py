"""
Ablation: train a CBM with a linear concept encoder.

This is a copy of train_cbm_suite.py with the nonlinearity removed from the
concept encoder: the ReLU between the two Linear layers of `proj_layer` is
dropped, so target features -> concepts is a linear map (composed of two
Linear layers, without an activation in between). Everything else (teacher
model, distillation) is unchanged.
"""
import sys
import os
...
def train_linear_cbm_and_save(config: TrainingConfig):
    """
    Ablation pipeline: linear concept encoder -> teacher model -> final layer.

    Identical to train_cbm_suite.py's pipeline except proj_layer has no ReLU
    between its two Linear layers (see build_linear_proj_layer below).
    """
    ...
    # ========== STAGE 1: Train Concept Encoder (linear: no ReLU) ==========
    print("\n" + "="*50)
    print("STAGE 1: Training Concept Encoder (linear)")
    print("="*50)
    ...
    save_name = "{}/{}_{}_linear".format(config.save_dir, config.backbone, datetime.datetime.now().strftime("%Y_%m_%d_%H_%M"))
    ...
    # NOTE: cbm.py's CBM_model currently rebuilds proj_layer WITH a ReLU, so its
    # state_dict keys won't line up with this linear (ReLU-free) proj_layer --
    # see the flag raised after this script.
...
if __name__=='__main__':
    ...
    train_linear_cbm_and_save(config)
